...
module.exports = class PriceFeedOracle {
  ...
  respond = async (queries) => {
    ...
    const response = await axios.get(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=100&ids=${queryString}`)
      .then(res => {
    ...
    if (response) {
      ...
      await this.oracle.respondToQuery(query.id, response, {responseTtl: query.responseTtl});
    } else {
